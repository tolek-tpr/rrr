...
RrrDevice::RrrDevice(RrrWindow &window) : window{window} {
  createInstance();
  setupDebugMessenger();
  createSurface();
  pickPhysicalDevice();
...
void RrrDevice::createLogicalDevice() {
...
void RrrDevice::createSurface() { window.createWindowSurface(instance, &surface_); }
...
std::vector<const char *> RrrDevice::getRequiredExtensions() {
...
QueueFamilyIndices RrrDevice::findQueueFamilies(VkPhysicalDevice device) {
...
void RrrDevice::createImageWithInfo(
